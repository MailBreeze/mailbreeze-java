...
/**
 * Base exception for all MailBreeze API errors. Contains structured error information from the API
 * response.
 */
public class MailBreezeException extends RuntimeException {
  ...
  public MailBreezeException(
      int statusCode, String code, String message, String requestId, Map<String, Object> details) {
  ...
  public static MailBreezeException fromStatusCode(
      int statusCode, String message, String requestId, Map<String, Object> details) {
    return switch (statusCode) {
      ...
      default -> {
        ...
        yield new MailBreezeException(
            statusCode, codeFromStatus(statusCode), message, requestId, details);
      }
    };
  }
  ...
  /**
   * Returns true if this error is potentially transient and the request could be retried. Only rate
   * limit errors (429) and server errors (5xx) are considered retryable.
   */
